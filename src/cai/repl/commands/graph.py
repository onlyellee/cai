...
class GraphCommand(Command):
    ...
    def __init__(self):
        ...
        super().__init__(
            name="/graph", description=t('graph_desc'), aliases=["/g"]
        )

        # Add subcommands
        self.add_subcommand("all", t('graph_sub_all'), self.handle_all)
        self.add_subcommand("timeline", t('graph_sub_timeline'), self.handle_timeline)
        self.add_subcommand("stats", t('graph_sub_stats'), self.handle_stats)
        self.add_subcommand("export", t('graph_sub_export'), self.handle_export)
    ...
    def _handle_single_agent_graph(self, agent_name: Optional[str] = None) -> bool:
            ...
            if not agent_name:
                ...
                if current_agent:
                    ...
                    if not active_agents:
                        console.print(f"[yellow]{t('graph_no_active_agent')}[/yellow]")
                        return True
        ...
        if not history:
            console.print(f"[yellow]{t('graph_no_history', agent_name=agent_name)}[/yellow]")
            return True
        ...
        try:
            ...
            console.print(f"\n[bold]{t('graph_conversation_for', agent_name=agent_name)}[/bold]")
            console.print("-" * (20 + len(agent_name)))

            if len(G.nodes) == 0:
                console.print(f"[yellow]{t('graph_no_messages')}[/yellow]")
            else:
            ...
            console.print(f"[red]{t('graph_error_display', error=e)}[/red]")
            return False
    ...
    def _handle_multi_agent_graph(self) -> bool:
        ...
        if os.getenv("CAI_TUI_MODE") == "true":
            try:
                ...
                if app and hasattr(app, 'session_manager') and app.session_manager.terminal_runners:
                    ...
                    if all_histories:
                        console.print(f"\n[bold cyan]{t('graph_terminal_agents_title')}[/bold cyan]")
                        console.print(Rule())
                        ...
                        for display_name, history in agents_to_show:
                            if not history:
                                ...
                                graphs.append(
                                    Panel(
                                        f"[dim]{t('graph_no_messages_yet')}[/dim]",
                        ...
                        console.print(f"\n[bold]{t('graph_summary')}[/bold]")
                        console.print(f"• {t('graph_total_terminals', count=len(all_histories))}")
                        console.print(f"• {t('graph_total_messages', count=total_messages)}")
                        console.print(f"• {t('graph_avg_messages_terminal', avg=f'{total_messages / len(all_histories) if all_histories else 0:.1f}')}")
        ...
        if not all_histories and not PARALLEL_CONFIGS:
            console.print(
                f"[yellow]{t('graph_no_agents_configured')}[/yellow]"
            )
            return True

        console.print(f"\n[bold cyan]{t('graph_multi_agent_title')}[/bold cyan]")
        console.print(Rule())
        ...
        for display_name, history in agents_to_show:
            ...
            try:
                ...
                if nodes:
                    ...
                    graph_lines.append(f"[dim]{t('graph_no_messages_yet')}[/dim]")
        ...
        console.print(f"• {t('graph_total_agents', count=len(agents_to_show))}")
        console.print(f"• {t('graph_total_messages', count=total_messages)}")
        console.print(
            f"• {t('graph_avg_messages_agent', avg=f'{total_messages / len(agents_to_show) if agents_to_show else 0:.1f}')}"
        )
    ...
    def handle_agent_graph(self, agent_id: str) -> bool:
                ...
                if f"P{idx}" == agent_id:
                    ...
                    if config.agent_name.endswith("_pattern"):
                        ...
                        if pattern and hasattr(pattern, "entry_agent"):
                            ...
                            console.print(
                                f"[yellow]{t('graph_pattern_not_found', name=config.agent_name)}[/yellow]"
                            )
                            return True
                        ...
                        console.print(f"[yellow]{t('graph_agent_not_found', name=config.agent_name)}[/yellow]")
                        return True
                    ...
                    if history:
                        ...
                        console.print(
                            f"[cyan]{t('graph_showing_for', agent_name=full_agent_name, agent_id=agent_id)}[/cyan]"
                        )
                        ...
                        try:
                            ...
                            console.print(
                                f"\n[bold]{t('graph_conversation_for', agent_name=full_agent_name)}[/bold]"
                            )
                        ...
                        console.print(
                            f"[yellow]{t('graph_no_history_for', agent_name=full_agent_name, agent_id=agent_id)}[/yellow]"
                        )
                        return True
        ...
        if not agent_name:
            console.print(f"[yellow]{t('graph_no_agent_with_id', agent_id=agent_id)}[/yellow]")
            console.print(f"[dim]{t('graph_use_history')}[/dim]")
            return True

        console.print(f"[cyan]{t('graph_showing_for', agent_name=agent_name, agent_id=agent_id)}[/cyan]")
        return self._handle_single_agent_graph(agent_name)
    ...
    def handle_timeline(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not all_histories:
            console.print(f"[yellow]{t('graph_no_conversation_history')}[/yellow]")
            return True
        ...
        table = Table(
            title=f"[bold cyan]{t('graph_timeline_title')}[/bold cyan]",
            show_header=True,
        ...
        console.print(table)
        console.print(f"\n[bold]{t('graph_total_events', count=len(timeline_events))}[/bold]")
        return True
    ...
    def handle_stats(self, args: Optional[List[str]] = None) -> bool:
        ...
        stats_table = Table(
            title=f"[bold cyan]{t('graph_stats_title')}[/bold cyan]",
            show_header=True,
            header_style="bold yellow",
        )
        ...
        if total_stats["total_messages"] > 0:
            ...
            console.print(
                f"• {t('graph_msg_distribution', user=f'{user_ratio:.1f}', assistant=f'{assistant_ratio:.1f}', tool=f'{tool_ratio:.1f}')}"
            )
            ...
            if total_stats.get("assistant", 0) > 0:
                ...
                console.print(
                    f"• {t('graph_avg_tool_calls', avg=f'{tools_per_assistant:.2f}')}"
                )

        console.print(f"• {t('graph_active_agents', count=len(all_histories))}")
        console.print(f"• {t('graph_total_conversations', count=sum(1 for h in all_histories.values() if h))}")
    ...
    def handle_export(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args:
            console.print(f"[yellow]{t('graph_export_format_required')}[/yellow]")
            console.print(t('graph_export_usage'))
            console.print(t('graph_export_formats'))
            return True
        ...
        if format_type not in ["json", "dot", "mermaid"]:
            console.print(f"[red]{t('graph_unknown_format', format=format_type)}[/red]")
            console.print(t('graph_supported_formats'))
            return False
        ...
        if not all_histories:
            console.print(f"[yellow]{t('graph_no_history_export')}[/yellow]")
            return True
        ...
        try:
            ...
            console.print(f"[green]{t('graph_export_success', filename=filename)}[/green]")

            # Show usage hints based on format
            if format_type == "dot":
                console.print(f"[dim]{t('graph_export_dot_hint', filename=filename)}[/dim]")
            elif format_type == "mermaid":
                console.print(f"[dim]{t('graph_export_mermaid_hint')}[/dim]")

        except Exception as e:
            console.print(f"[red]{t('graph_export_error', error=e)}[/red]")
            return False
    ...
    def _build_compact_graph(self, history: list) -> list:
        ...
        for idx, msg in enumerate(history):
            ...
            if role == "user":
                ...
                # Check if it has tool calls
                if msg.get("tool_calls"):
                    graph_lines.append(f"[bold red][{turn_counter}][/bold red] [yellow]▶ Agent (tools)[/yellow]")
                else:
                ...
                graph_lines.append(f"[yellow]▶ {role}[/yellow]")
            
            # Add arrow between messages
            if idx < len(history) - 1 and history[idx + 1].get("role") != "system":
                graph_lines.append("    ↓")
        
        return graph_lines if graph_lines else [f"[dim]{t('graph_no_messages_yet')}[/dim]"]

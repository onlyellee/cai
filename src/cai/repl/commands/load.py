...
class LoadCommand(Command):
    ...
    def __init__(self):
        ...
        super().__init__(
            name="/load",
            description=t('load_desc'),
            aliases=["/l"],
        )

        # Add subcommands
        self.add_subcommand("agent", t('load_sub_agent'), self.handle_agent)
        self.add_subcommand("all", t('load_sub_all'), self.handle_all)
        self.add_subcommand(
            "parallel", t('load_sub_parallel'), self.handle_parallel
        )
        self.add_subcommand(
            "load-all",
            t('load_sub_load_all'),
    ...
    def handle(self, args: Optional[List[str]] = None) -> bool:
                ...
                if terminal_id:
                    ...
                    if terminal_id.startswith("terminal-") and terminal_id.count("-") >= 2:
                        ...
                        if len(parts) >= 3 and parts[2].isdigit():
                            ...
                            console.print(f"[cyan]{t('load_loading_terminal', num=terminal_num)}[/cyan]")
        ...
        if pattern and hasattr(pattern, "configs"):
            ...
            console.print(f"[green]{t('load_loaded_pattern', desc=pattern.description)}[/green]")
            console.print(f"[cyan]{t('load_agents_configured', count=len(PARALLEL_CONFIGS))}[/cyan]")
            ...
            if not loaded:
                console.print(f"[yellow]{t('load_no_history_from', file=jsonl_file)}[/yellow]")
        ...
        if "/" in args[0] or "." in args[0] or args[0].endswith(".jsonl"):
            ...
            if os.getenv("CAI_TUI_MODE") == "true":
                ...
                if hasattr(terminal_tracking._thread_local, 'terminal_number'):
                    terminal_num = terminal_tracking._thread_local.terminal_number
                    console.print(f"[cyan]{t('load_loading_file_terminal', num=terminal_num)}[/cyan]")
                ...
                # Not in TUI mode, load into default agent (P1)
        ...
        if args[0].upper().startswith("P"):
            ...
            if os.getenv("CAI_TUI_MODE") == "true":
                ...
                if terminal_number.isdigit():
                    ...
                    if agent_name:
                        console.print(f"[cyan]{t('load_loading_agent', num=terminal_num, agent=agent_name)}[/cyan]")
                    else:
                        # In TUI mode, we can still load using P-ID directly
                        console.print(f"[yellow]{t('load_loading_terminal_pid', num=terminal_num)}[/yellow]")
            ...
            # Check if there are no parallel configs
                ...
                if identifier == "P1":
                    ...
                    if current_agent and current_agent_name:
                        ...
                        console.print(f"[cyan]{t('load_loading_current', agent=agent_name)}[/cyan]")
                    else:
                        console.print(f"[red]{t('load_error_no_active')}[/red]")
                        return False
                else:
                    # Any other ID in single agent mode is invalid
                    console.print(f"[red]{t('load_error_no_agent_id', id=identifier)}[/red]")
                    console.print(f"[yellow]{t('load_single_only_p1')}[/yellow]")
                    console.print(f"[dim]{t('load_use_parallel')}[/dim]")
                    return False
            else:
            ...
            if agent_name:
                ...
                console.print(f"[red]{t('load_error_no_agent_id', id=identifier)}[/red]")
                console.print(f"[dim]{t('load_use_parallel_ids')}[/dim]")
                return False
    ...
    def handle_load_pattern_from_jsonl(self, jsonl_file: Optional[str] = None) -> bool:
        ...
        try:
            ...
            console.print(f"[green]{t('load_loaded_messages', count=len(messages), file=jsonl_file)}[/green]")

            # Debug: Show what agent names were found
            if agent_conversations:
                console.print(f"[dim]{t('load_found_conversations')}[/dim]")
            ...
            for idx, config in enumerate(PARALLEL_CONFIGS, 1):
                ...
                if agent_counts[config.agent_name] > 1:
                    ...
                    if best_match:
                        ...
                        console.print(
                            f"[green]{t('load_loaded_pattern_messages', count=best_count, agent=instance_name, id=f'P{idx}')}[/green]"
                        )
                        loaded_count += 1

            if loaded_count > 0:
                console.print(
                    f"[bold green]{t('load_success_agents', count=loaded_count)}[/bold green]"
                )

                # Final sync to ensure all histories are visible
                if PARALLEL_ISOLATION.is_parallel_mode():
                    console.print(f"[dim]{t('load_syncing')}[/dim]")
                    PARALLEL_ISOLATION.sync_with_agent_manager()
            else:
                console.print(f"[yellow]{t('load_no_matching')}[/yellow]")

                # If no agents were found, provide helpful information
                if not agent_conversations:
                    console.print(
                        f"[dim]{t('load_empty_jsonl')}[/dim]"
                    )
                    console.print(
                        f"[dim]{t('load_agent_fields_hint')}[/dim]"
                    )
                    return False
                else:
                    console.print(f"\n[dim]{t('load_found_agents_jsonl')}[/dim]")
                    ...
                    console.print(f"\n[dim]{t('load_configured_expecting')}[/dim]")
                    for idx, config in enumerate(PARALLEL_CONFIGS, 1):
                    ...
                    console.print(
                        f"\n[dim]{t('load_tip_names_match')}[/dim]"
                    )

            return True

        except Exception as e:
            console.print(f"[red]{t('load_error_pattern', error=str(e))}[/red]")
            return False
    ...
    def handle_load_default(self, jsonl_file: Optional[str] = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                console.print(f"[red]{t('load_error_file_not_found', file=jsonl_file)}[/red]")
                return False
            except Exception as e:
                console.print(f"[red]{t('load_error_loading', file=jsonl_file, error=e)}[/red]")
                return False

            # Check if there are any messages to load
            if not messages:
                console.print(f"[yellow]{t('load_no_messages_file', file=jsonl_file)}[/yellow]")
                return True
            ...
            if os.getenv("CAI_TUI_MODE") == "true":
                ...
                if terminal_id:
                    ...
                    if terminal_num:
                        p_id = f"P{terminal_num}"
                        console.print(f"[cyan]{t('load_loading_terminal_p', num=terminal_num, id=p_id)}[/cyan]")
                        return self._load_to_agent(p_id, jsonl_file)

            if not current_agent:
                console.print(f"[red]{t('load_error_no_active_agent')}[/red]")
                console.print(f"[yellow]{t('load_select_agent_hint')}[/yellow]")
                return False
            ...
            if success:
                console.print(
                    f"[green]{t('load_session_loaded')}[/green]"
                )

            return success

        except Exception as e:  # pylint: disable=broad-exception-caught
            console.print(f"[red]{t('load_error_jsonl', error=str(e))}[/red]")
            return False
    ...
    def handle_agent(self, args: Optional[List[str]] = None) -> bool:
        ...
        """
        if not args:
            console.print(f"[red]{t('load_error_agent_required')}[/red]")
            console.print(t('load_usage_agent'))
            console.print(t('load_usage_agent_example'))
            console.print(t('load_usage_agent_example2'))
            return False
    ...
    def _load_to_agent(self, agent_name: str, jsonl_file: str) -> bool:
        ...
        try:
            ...
            if not messages:
                console.print(f"[yellow]{t('load_no_messages_file', file=jsonl_file)}[/yellow]")
                console.print(f"[dim]{t('load_file_empty_hint')}[/dim]")
                return True
            ...
            if (
                ...
                if os.getenv("CAI_TUI_MODE") == "true":
                    ...
                    console.print(f"[cyan]{t('load_loading_terminal_p', num=agent_id[1:], id=agent_id)}[/cyan]")
                    ...
                    if resolved_name:
                        ...
                        console.print(f"[cyan]{t('load_resolved_agent', id=agent_id, agent=resolved_agent_name)}[/cyan]")
                    else:
                        console.print(f"[red]{t('load_error_no_agent_id', id=agent_id)}[/red]")
                        console.print(f"[yellow]{t('load_available_agents')}[/yellow]")
            ...
            is_active_agent = (
                current_agent and
                (current_agent_name == resolved_agent_name or
                 resolved_agent_name == "P1" or
                 (hasattr(current_agent, 'name') and current_agent.name == resolved_agent_name))
            )
            ...
            if is_active_agent and current_agent:
                ...
                if success:
                    console.print(
                        f"[green]{t('load_session_loaded_agent', agent=resolved_agent_name)}[/green]"
                    )
                return success
            else:
                ...
                if model_instance:
                    ...
                    for msg in normalized_messages:
                        model_instance.add_to_message_history(msg, skip_deduplication=True)
                    console.print(f"[green]{t('load_updated_model')}[/green]")
                else:
                ...
                console.print(
                    f"[green]{t('load_loaded_into_agent', count=len(normalized_messages), agent=resolved_agent_name)}[/green]"
                )
    ...
    def handle_all(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not all_agents:
            console.print(f"[yellow]{t('load_no_agents_init')}[/yellow]")
            console.print(
                f"[dim]{t('load_agents_created_hint')}[/dim]"
            )
            console.print(f"[dim]{t('load_agents_config_hint')}[/dim]")
            return True
        ...
        table = Table(
            title=t('load_table_title'),
            show_header=True,
            header_style="bold yellow",
        )
        table.add_column(t('load_col_id'), style="magenta", width=4)
        table.add_column(t('load_col_agent_name'), style="cyan")
        table.add_column(t('load_col_current_messages'), style="green", justify="right")
        table.add_column(t('load_col_message_types'), style="magenta")
        table.add_column(t('load_col_status'), style="yellow")
        ...
        for agent_name in sorted(all_agents):
            ...
            if history:
                ...
                status = t('load_status_active')
            else:
                role_str = t('load_no_messages')
                status = t('load_status_configured') if agent_name in configured_agents else t('load_status_empty')
        ...
        console.print(table)
        console.print(f"\n[dim]{t('load_usage_load_agent')}[/dim]")
        console.print(f"[dim]{t('load_usage_load_id')}[/dim]")
        console.print(f"[dim]{t('load_usage_load_all')}[/dim]")
        console.print(f"[dim]{t('load_example_agent')}[/dim]")
        console.print(f"[dim]{t('load_example_bug')}[/dim]")
        console.print(f"[dim]{t('load_example_id')}[/dim]")
        console.print(f"[dim]{t('load_example_all')}[/dim]")
    ...
    def handle_load_all(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not PARALLEL_CONFIGS:
            console.print(f"[yellow]{t('load_no_parallel')}[/yellow]")
            console.print(f"[dim]{t('load_no_parallel_hint')}[/dim]")
            return False
        ...
        try:
            ...
            for idx, config in enumerate(PARALLEL_CONFIGS, 1):
                if config.agent_name in available_agents:
                    ...
                    console.print(f"[green]{t('load_loaded_into', agent=instance_name, id=agent_id)}[/green]")

            console.print(
                f"\n[bold green]{t('load_loaded_all_success', count=len(messages), agents=len(loaded_agents))}[/bold green]"
            )

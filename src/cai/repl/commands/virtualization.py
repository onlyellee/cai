        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if "Mounts denied" in error_msg or try_mount:
                    console.print(
                        f"[yellow]{t('virt_mount_failed_retry')}[/yellow]"
                    )
                ...
                if "already exists" in error_msg and container_name:
                    ...
                    for container in DockerManager.get_container_list():
                        if container.get("Names", "") == f"/{container_name}":
                            if container.get("Image", "") == image_name:
                                ...
                                if start_process.returncode == 0:
                                    ...
                                    if not verify_result.stdout.strip():
                                        ...
                                        console.print(
                                            f"[yellow]{t('virt_container_exits_immediately')}[/yellow]"
                                        )
                                    ...
                                    if needs_entrypoint_override:
                                        ...
                                        console.print(
                                            f"[yellow]{t('virt_cannot_start_removing')}[/yellow]"
                                        )
                            ...
                            console.print(
                                f"[yellow]{t('virt_container_name_conflict').format(name=container_name)}[/yellow]"
                            )
    ...
    def set_active_container(container_id: str) -> None:
        ...
        try:
            ...
            if check_process.returncode == 0 and "false" in check_process.stdout.lower():
                console.print(
                    f"[yellow]{t('virt_container_exist_not_running').format(id=container_id[:12])}[/yellow]"
                )
                ...
                if start_process.returncode != 0:
                    console.print(
                        f"[yellow]{t('virt_normal_start_failed_keepalive')}[/yellow]"
                    )
                    ...
                    if not verify.stdout.strip():
                        console.print(
                            f"[yellow]{t('virt_container_started_exited')}[/yellow]"
                        )
            ...
            console.print(f"[yellow]{t('virt_warn_container_activation').format(error=str(e))}[/yellow]")
        ...
        try:
            ...
            if check_running.returncode == 0 and "true" in check_running.stdout.lower():
                ...
                if mkdir_result.returncode == 0:
                    console.print(
                        f"[dim]{t('virt_created_workspace_container').format(path=container_workspace_path)}[/dim]"
                    )
                else:
                    console.print(
                        f"[yellow]{t('virt_warn_workspace_container').format(error=mkdir_result.stderr)}[/yellow]"
                    )
        except Exception as e:
            console.print(
                f"[yellow]{t('virt_warn_workspace_setup').format(error=str(e))}[/yellow]"
            )
...
class VirtualizationCommand(Command):
    ...
    def __init__(self):
        ...
        super().__init__(
            name="/virtualization",
            description=t('virt_description'),
            aliases=["/virt"],
        )
        ...
        # Add subcommands
        self.add_subcommand("pull", t('virt_sub_pull'), self.handle_pull_subcommand)
        self.add_subcommand("run", t('virt_sub_run'), self.handle_run_subcommand)
    ...
    def show_virtualization_status(self) -> bool:
        ...
        if not docker_manager.is_docker_installed():
            console.print(
                Panel(
                    t('virt_docker_not_found_body'),
                    title=t('virt_docker_not_found_title'),
                    border_style="red",
                )
            )
            return True
        ...
        if not docker_manager.is_docker_running():
            console.print(
                Panel(
                    t('virt_docker_not_running_body'),
                    title=t('virt_docker_not_running_title'),
                    border_style="yellow",
                )
            )
            return True
    ...
    def show_active_environment(self, active_container: str) -> None:
        ...
        if active_container:
            env_name = t('virt_docker_container')
            ...
            if image_name in DEFAULT_IMAGES:
                ...
                if category:
                    env_name = f"{category} {t('virt_environment')}"
                    if image_id:
            ...
            title = f"{t('virt_active_env_title')} {icon} {env_name}"
            border_style = "green"
        else:
            env_name = t('virt_host_system')
            env_info = t('virt_host_system_info')
            title = f"{t('virt_active_env_title')} 💻 {t('virt_host_system')}"
            border_style = "blue"

        # Show the panel with environment information
        panel_content = [
            f"[bold cyan]{t('virt_active_env')}[/bold cyan] [bold green]{env_name}[/bold green]",
            f"[cyan]{t('virt_details')}[/cyan] {env_info}",
            "",
            f"[bold yellow]{t('virt_select_env')}[/bold yellow]",
        ]
    ...
    def show_useful_images_table(self, show_all: bool = False) -> None:
        ...
        image_table = Table(
            title=t('virt_table_available_envs'),
            show_header=True,
        ...
        image_table.add_column(t('virt_col_id'), style="bold blue", justify="center", width=6)
        image_table.add_column(t('virt_col_environment'), style="bold cyan", width=30)
        image_table.add_column(t('virt_col_description'), style="white")
        image_table.add_column(t('virt_col_status'), style="green", justify="center", width=20)

        # Add Host System as the first option
        is_host_active = not active_container
        host_status = (
            f"[bold green]{t('virt_current_env')}[/bold green]"
            if is_host_active
            else f"[dim]{t('virt_available')}[/dim]"
        )
        display_host = (
            f"💻 [bold green]{t('virt_host_active')} ⭐ ACTIVE[/bold green]"
            if is_host_active
            else f"💻 {t('virt_host_system')}"
        )

        image_table.add_row(
            "[bold]host[/bold]",
            display_host,
            t('virt_host_system_desc'),
            host_status,
        )
        ...
        for image_name in essential_images:
            if image_name in DEFAULT_IMAGES:
                ...
                # Check image status
                status = f"[dim]{t('virt_not_pulled')}[/dim]"
                container_id = ""
                ...
                for container in self.cached_containers:
                    if container.get("Image", "") == image_name:
                        ...
                        if "up" in container_status:
                            status = f"[bold green]{t('virt_running')}[/bold green] (ID: {container_id})"
                        elif "exited" in container_status or "created" in container_status:
                            status = f"[yellow]{t('virt_stopped')}[/yellow] (ID: {container_id})"
                        else:
                            status = f"[blue]{t('virt_container')}[/blue] (ID: {container_id})"
                        break
                ...
                if not container_id:
                    if image_name in available_images:
                        ...
                        status = f"[green]{t('virt_available')}[/green] (ID: {image_id_short})"
                ...
                if image_name == active_image:
                    ...
                    status = f"[bold green]{t('virt_current_env')}[/bold green]"
    ...
    def _show_all_images_by_category(self, available_images, active_image):
        ...
        console.print(f"\n[bold cyan]{t('virt_all_available_envs')}[/bold cyan]\n")
        ...
        for category in category_order:
            ...
            image_table = Table(
                title=t('virt_category_envs').format(category=category),
                show_header=True,
                header_style="bold yellow",
                title_style="bold cyan",
                box=rich.box.SIMPLE,
            ...
            for name, info in images:
                ...
                if not container_id:
                    if name in available_images:
                        ...
                        status = f"[green]{t('virt_available')}[/green] (ID: {image_id})"
    ...
    def show_usage_info(self) -> None:
        ...
        console.print(f"\n[bold cyan]{t('virt_available_commands')}[/bold cyan]")
        console.print(
            f"  [bold]/virtualization[/bold]                      - "
            f"{t('virt_cmd_show_menu')}"
        )
        console.print(
            f"  [bold]/virtualization <image_name>[/bold]         - {t('virt_cmd_switch_name')}"
        )
        console.print(
            f"  [bold]/virtualization <image_id>[/bold]           - "
            f"{t('virt_cmd_switch_id')}"
        )

        # Common commands with examples
        console.print(f"\n[bold yellow]{t('virt_common_commands')}[/bold yellow]")
        console.print(
            f"  [bold]/virtualization kalilinux/kali-rolling[/bold] - "
            f"{t('virt_cmd_kali')}"
        )
        console.print(
            f"  [bold]/virtualization parrotsec/security[/bold]   - "
            f"{t('virt_cmd_parrot')}"
        )
        console.print(
            f"  [bold]/virtualization host[/bold]                 - {t('virt_cmd_host')}"
        )

        # General note
        console.print(f"\n[bold red]{t('virt_important')}[/bold red]")
        console.print(
            f"[dim]{t('virt_important_note')}[/dim]"
        )
    ...
    def handle_pull_subcommand(self, args: Optional[List[str]] = None) -> bool:
        ...
        """
        if not args:
            console.print(f"[yellow]{t('virt_please_specify_pull')}[/yellow]")
            return False
        ...
        # Check Docker status
        if not docker_manager.is_docker_installed():
            console.print(f"[red]{t('virt_docker_not_installed')}[/red]")
            return False

        if not docker_manager.is_docker_running():
            console.print(f"[yellow]{t('virt_docker_not_running')}[/yellow]")
            return False
    ...
    def handle_run_subcommand(self, args: Optional[List[str]] = None) -> bool:
        ...
        """
        if not args:
            console.print(f"[yellow]{t('virt_please_specify_run')}[/yellow]")
            return False
        ...
        if success:
            ...
            console.print(
                f"[green]{t('virt_container_set_active').format(id=container_id[:12])}[/green]"
            )
    ...
    def handle_activate_image(self, image_identifier: str) -> bool:
        ...
        if image_identifier.lower() in ["host", "system", "none"]:
            if "CAI_ACTIVE_CONTAINER" in os.environ:
                ...
                console.print(
                    f"[green]{t('virt_switched_host').format(id=previous[:12])}[/green]"
                )
                _sync_tui_container_selection("")
                return True
            else:
                console.print(f"[yellow]{t('virt_already_host')}[/yellow]")
                _sync_tui_container_selection("")
                return True
        ...
        for container in self.cached_containers:
            ...
            if container_id.startswith(image_identifier):
                ...
                console.print(f"[yellow]{t('virt_found_container').format(id=container_id[:12])}[/yellow]")
                ...
                if "exited" in container_status or "created" in container_status:
                    console.print(
                        f"[yellow]{t('virt_container_stopped_starting').format(id=container_id[:12])}[/yellow]"
                    )
                ...
                console.print(
                    f"[green]{t('virt_switched_container').format(id=container_id[:12], image=image_name)}[/green]\n"
                    f"[dim]{t('virt_commands_in_container').format(category=category)}[/dim]"
                )
        ...
        if image_identifier in self.id_to_image:
            ...
            console.print(f"[dim]{t('virt_using_image_for_id').format(image=image_name, id=image_identifier)}[/dim]")
        ...
        for container in self.cached_containers:
            ...
            if container_image == normalize_image_name(image_name):
                ...
                if "exited" in container_status or "created" in container_status:
                    ...
                    console.print(
                        f"[yellow]{t('virt_container_stopped_activating').format(id=container_id[:12])}[/yellow]"
                    )
                    ...
                    try:
                        ...
                        if start_process.returncode != 0:
                            console.print(
                                f"[yellow]{t('virt_normal_start_failed').format(error=start_process.stderr.strip())}[/yellow]"
                            )
                            console.print(
                                f"[yellow]{t('virt_setting_active_anyway')}[/yellow]"
                            )
                            ...
                            console.print(
                                f"[yellow]{t('virt_container_not_running').format(id=container_id[:12])}[/yellow]\n"
                                f"[dim]{t('virt_commands_host_fallback')}[/dim]"
                            )
                ...
                console.print(
                    f"[green]{t('virt_switched_env').format(category=category)}[/green]\n"
                    f"[dim]{t('virt_using_existing').format(id=container_id[:12])}[/dim]"
                )
        ...
        if not image_available:
            console.print(
                f"[yellow]{t('virt_image_not_found').format(image=image_name)}[/yellow]"
            )
        ...
        console.print(
            f"[green]{t('virt_switched_env_image').format(category=category, image=image_name)}[/green]\n"
            f"[dim]{t('virt_commands_in_new_container').format(id=container_id[:12])}[/dim]"
        )
    ...
    def _handle_parrotsec(self) -> bool:
        ...
        console.print(f"[yellow]{t('virt_special_handling').format(image=image_name)}[/yellow]")
        ...
        if not fixed_image_exists:
            console.print(f"[yellow]{t('virt_creating_fixed_image')}[/yellow]")
            ...
            if not original_exists:
                console.print(f"[yellow]{t('virt_pulling_image').format(image=image_name)}[/yellow]")
                with console.status(f"Pulling {image_name}..."):
                    ...
                    if result.returncode != 0:
                        console.print(f"[red]{t('virt_failed_pull').format(image=image_name, error=result.stderr)}[/red]")
                        return False
            ...
            with console.status("Creating temporary container..."):
                ...
                if create_result.returncode != 0:
                    console.print(
                        f"[red]{t('virt_failed_create_temp').format(error=create_result.stderr)}[/red]"
                    )
                    return False
...
"""
            ...
            console.print(
                f"[green]{t('virt_created_fixed_image').format(name=fixed_image_name)}[/green]"
            )
        ...
        if existing_container_id:
            console.print(f"[yellow]{t('virt_found_existing_fixed')}[/yellow]")
            ...
            if check_result.returncode == 0 and "true" in check_result.stdout.lower():
                ...
                console.print(
                    f"[green]{t('virt_container_already_running').format(id=existing_container_id[:12])}[/green]"
                )
            else:
                # Start it
                ...
                if start_result.returncode != 0:
                    ...
                    console.print(
                        f"[yellow]{t('virt_failed_start_recreating')}[/yellow]"
                    )
                    ...
                    console.print(
                        f"[green]{t('virt_container_started').format(id=existing_container_id[:12])}[/green]"
                    )

        # Step 5: If no running container, create a new one
        if not existing_container_id:
            console.print(f"[yellow]{t('virt_creating_new_container')}[/yellow]")
            ...
            with console.status("Creating container..."):
                ...
                if create_result.returncode != 0:
                    console.print(f"[red]{t('virt_failed_create_container').format(error=create_result.stderr)}[/red]")
                    return False
                ...
                if not check_result.stdout.strip():
                    console.print(
                        f"[red]{t('virt_container_created_not_running')}[/red]"
                    )
                else:
                    console.print(
                        f"[green]{t('virt_container_created_running').format(id=existing_container_id[:12])}[/green]"
                    )
        ...
        console.print(
            f"[green]{t('virt_switched_env_image').format(category=category, image=image_name)}[/green]\n"
            f"[dim]{t('virt_commands_in_new_container').format(id=existing_container_id[:12])}[/dim]"
        )
    ...
    def _fallback_to_best_environment(self) -> bool:
        ...
        for container in self.cached_containers:
            ...
            if image in DEFAULT_IMAGES:
                ...
                console.print(
                    f"[yellow]{t('virt_fallback_existing').format(id=container_id[:12], image=image)}[/yellow]"
                )
                # Refresh Docker info
                self.refresh_docker_info()
        ...
        if "kalilinux/kali-rolling" in [img.get("Repository", "") for img in self.cached_images]:
            console.print(f"[yellow]{t('virt_fallback_kali')}[/yellow]")

...
class CommandHandler:
    ...
    def handle_command(self, command: str) -> None:
        ...
        if (
            ...
            if self.output:
                self.output.write(
                    f"[yellow]{t('tui_compaction_wait')}[/yellow]"
                )
            return
        ...
        try:
            ...
            if not handled:
                if suggested:
                    intercept_console.print(f"[red]{t('tui_unknown_command', cmd=cmd_name)}[/red]")
                    intercept_console.print(f"[yellow]{t('tui_did_you_mean', suggested=suggested)}[/yellow]")
                else:
                    intercept_console.print(f"[red]{t('tui_unknown_command', cmd=cmd_name)}[/red]")
            ...
            if cmd_name.lower() in ["/agent", "/a"] and args:
                ...
                known_subs = {"select", "list", "info", "multi", "current", "new"}

...
class CTRCanvas(Container):
    ...
    #ctr-toolbar Button:last-child { margin-right: 0; }
    ...
    def compose(self) -> ComposeResult:
        with Horizontal(id="ctr-toolbar"):
            yield Label(t('tui_graph_run'))
            yield Select([], id="run-select", prompt=t('tui_graph_select_run'))
            yield Button(t('tui_graph_reload'), id="reload-run")
            yield Button(t('tui_graph_layout'), id="relayout")
            yield Button(t('tui_graph_zoom_in'), id="zoom_in")
            yield Button(t('tui_graph_zoom_out'), id="zoom_out")
            yield Button(t('tui_graph_fit'), id="fit")
            yield Button(t('tui_clear'), id="clear")

        with Horizontal(id="workspace"):
            with Vertical(id="left"):
                with Container(id="canvas"):
                    yield GraphViewport(id="viewport")
                yield Static(t('tui_graph_ready'), id="status")
            with Vertical(id="right"):
                yield Static(t('tui_graph_details'), id="details-title")

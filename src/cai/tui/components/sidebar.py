...
class AddApiKeyDialog(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="add-key-overlay"):
            with Vertical(id="add-key-content"):
                # Header
                yield Label(t('tui_add_api_key'), id="add-key-header")
                
                # Form
                with Vertical(id="add-key-form"):
                    # Model field
                    yield Label(t('tui_provider_label'), classes="form-label")
                    yield Input(placeholder=t('tui_provider_placeholder'), id="model-input")
                    
                    # Small spacer
                    yield Static("", classes="form-spacer")
                    
                    # API Key field  
                    yield Label(t('tui_api_key_label'), classes="form-label")
                ...
                with Horizontal(id="add-key-buttons"):
                    yield Button(t('tui_cancel'), id="cancel-btn", classes="dialog-cancel-btn")
                    yield Button(t('tui_save'), id="save-btn", classes="dialog-save-btn")
    ...
    def _save_api_key(self):
        ...
        if not model and not api_key:
            self._show_error_message(t('tui_both_fields_required'))
            return
        elif not model:
            self._show_error_message(t('tui_provider_required'))
            return
        elif not api_key:
            self._show_error_message(t('tui_api_key_required'))
            return
    ...
    def compose(self) -> ComposeResult:
        with Container(id="agent-action-overlay"):
            with Container(id="agent-action-content"):
                yield Label(t('tui_choose_action'), id="agent-action-header")
                with Container(id="agent-action-buttons"):
                    yield Button(t('tui_update_t1'), id="update-terminal-btn", classes="dialog-action-btn")
                    yield Button(t('tui_new_terminal'), id="new-terminal-btn", classes="dialog-action-btn")
                    yield Button(t('tui_cancel'), id="cancel-action-btn", classes="dialog-action-btn")
...
class Sidebar(Container):
    ...
    def refresh_queue(self) -> None:
        ...
        if not queue:
            # Show empty message
            empty_msg = Static(t('tui_queue_empty'), classes="queue-empty")
    ...
    def refresh_state(self) -> None:
        ...
        try:
            ...
            if not self._stats_widgets:
                ...
                self._stats_widgets['agent_title'] = Static(f"🤖 {t('tui_agents_section')}", classes="state-section-title")
                self._stats_widgets['agent_section'] = Vertical(
                    self._stats_widgets['agent_title'],
                    classes="state-section"
                )
                
                self._stats_widgets['system_title'] = Static(f"⚙️ {t('tui_system_section')}", classes="state-section-title")
            ...
            if display_cost != "--":
                system_section.mount(Static(f"{t('tui_cost_label')}: [yellow]{display_cost}[/yellow]", classes="state-content"))
            else:
                system_section.mount(Static(f"{t('tui_cost_label')}: [dim]--[/dim]", classes="state-content"))
            ...
            if terminal_costs_map:
                ...
                if terminal_lines:
                    ...
                    system_section.mount(Static(f"[dim]{t('tui_per_terminal')}[/dim]", classes="state-content"))
            ...
            if unattributed_cost > 0.00005:
                system_section.mount(
                    Static(
                        f"{t('tui_unattributed')}: [yellow]${unattributed_cost:.4f}[/yellow]",
                        classes="state-content",
                    )
                )
            system_section.mount(Static(f"{t('tui_mode_label')}: [cyan]{current_state['mode']}[/cyan]", classes="state-content"))
            ...
            if meta and meta.get('enabled'):
                ...
                system_section.mount(Static(f"{t('tui_meta_agent')}: [green]ON[/green]", classes="state-content"))
                system_section.mount(Static(f"{t('tui_last_agent')}: [cyan]{last_agent}[/cyan]", classes="state-content"))
                system_section.mount(Static(f"{t('tui_parallel_label')}: [magenta]{parallel}[/magenta]", classes="state-content"))
            ...
            if current_state['memory_count'] > 0:
                if 'memory_section' not in self._stats_widgets:
                    self._stats_widgets['memory_title'] = Static(f"🧠 {t('tui_memory_section')}", classes="state-section-title")
                ...
                for child in list(memory_section.children)[1:]:
                    child.remove()
                memory_section.mount(Static(f"{t('tui_memory_active', count=current_state['memory_count'])}", classes="state-content"))
            ...
            if current_state['queue_count'] > 0:
                if 'queue_section' not in self._stats_widgets:
                    self._stats_widgets['queue_title'] = Static(f"📋 {t('tui_queue_stats_section')}", classes="state-section-title")
                ...
                for child in list(queue_section.children)[1:]:
                    child.remove()
                queue_section.mount(Static(f"{t('tui_queue_items')}: [green]{current_state['queue_count']}[/green]", classes="state-content"))
    ...
    def refresh_keys(self) -> None:
        ...
        try:
            ...
            if not api_keys:
                # Show empty state
                empty_label = Label(t('tui_no_api_keys'), classes="keys-empty")
            ...
            add_button = Button(t('tui_add_new_key'), classes="key-add-button")
            ...
            try:
                if len(self.keys_container.children) == 0:
                    error_label = Label(t('tui_error_loading_keys', error=str(e)[:50]), classes="keys-empty")
